...
async fn upload_folder(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    match config.protocol.as_str() {
        "ftp" => do_upload_ftp(app, config, false),
        "ftps" => do_upload_ftp(app, config, true),
...
fn do_upload_ftp(app: AppHandle, config: UploadConfig, use_tls: bool) -> Result<(), String> {
    ...
    if use_tls {
        ...
        let connector = builder.build().map_err(|e| format!("Errore TLS: {}", e))?;
