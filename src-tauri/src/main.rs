...
async fn upload_folder(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    match config.protocol.as_str() {
        "ftp" => do_upload_ftp_plain(app, config),
...
fn do_upload_ftp_plain(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    ...
    for entry in files {
        ...
        if let Some(parent) = Path::new(&remote_file_path).parent() {
            ...
            create_ftp_dirs_plain(&mut ftp, &parent_str)?;
        }
...
fn create_ftp_dirs_plain(ftp: &mut FtpStream, path: &str) -> Result<(), String> {
...
fn do_upload_ftps(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    ...
    let mut ftp = FtpStream::connect(&address)
        ...
        .map_err(|e| format!("Connessione FTPS fallita: {}. Prova ad attivare 'Accetta certificati non verificati'.", e))?;
...
fn create_ftps_dirs(ftp: &mut suppaftp::FtpStream<suppaftp::NativeTlsStream>, path: &str) -> Result<(), String> {
    let parts: Vec<&str> = path.split('/').filter(|s| !s.is_empty()).collect();
    let mut current_path = String::new();
    for part in parts {
        current_path.push('/');
