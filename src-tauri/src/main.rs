...
fn do_upload_ftp(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    ...
    for entry in files {
        ...
        // TEST: forza errore su file .php
        let status = if filename.ends_with(".php") {
            eprintln!("TEST: forced error for {}", filename);
            "error"
        } else {
            match ftp.put_file(&remote_file_path, &mut content.as_slice()) {
                Ok(_) => {
...
fn do_upload_sftp(app: AppHandle, config: UploadConfig) -> Result<(), String> {
    ...
    for entry in files {
        ...
        let status = if filename.ends_with(".php") {
            ...
            match sftp.create(Path::new(&remote_file_path)) {
