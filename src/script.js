...
let invoke, open, save, listen, message;
try {
...
// Native system dialog (macOS NSAlert / Windows TaskDialog / Linux)
async function showMessage(msg, kind = 'warning') {
    try { await message(msg, { title: 'iCONup', kind }); }
...
const productPaths = { iconsite: '/', iconcms: '/_iconcms/', iconstat: '/_iconstat/', iconblog: '/_iconblog/', iconvert: '/_iconcms/iconvert/', custom: '' };

// Inline SVG icons (Material Symbols, currentColor) — replace emoji glyphs
const SVG_CHEVRON_LEFT = '<svg class="nav-chevron" viewBox="0 -960 960 960" width="18" height="18" fill="currentColor"><path d="M560-240 320-480l240-240 56 56-184 184 184 184-56 56Z"/></svg>';
const SVG_CHEVRON_RIGHT = '<svg class="nav-chevron" viewBox="0 -960 960 960" width="18" height="18" fill="currentColor"><path d="M504-480 320-664l56-56 240 240-240 240-56-56 184-184Z"/></svg>';
const SVG_FILE = '<svg viewBox="0 -960 960 960" width="16" height="16" fill="currentColor"><path d="M320-240h320v-80H320v80Zm0-160h320v-80H320v80Zm-80 320q-33 0-56.5-23.5T160-160v-640q0-33 23.5-56.5T240-880h280l240 240v440q0 33-23.5 56.5T680-80H240Zm240-520v-200H240v640h440v-440H480ZM240-800v200-200 640-640Z"/></svg>';
const SVG_CHECK = '<svg viewBox="0 -960 960 960" width="16" height="16" fill="currentColor"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>';
const SVG_CLOSE = '<svg viewBox="0 -960 960 960" width="16" height="16" fill="currentColor"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"/></svg>';
const SVG_RESULT_CHECK = '<svg viewBox="0 -960 960 960" width="44" height="44" fill="currentColor"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>';
const SVG_RESULT_WARNING = '<svg viewBox="0 -960 960 960" width="44" height="44" fill="currentColor"><path d="m40-120 440-760 440 760H40Zm138-80h604L480-720 178-200Zm302-40q17 0 28.5-11.5T520-280q0-17-11.5-28.5T480-320q-17 0-28.5 11.5T440-280q0 17 11.5 28.5T480-240Zm-40-120h80v-200h-80v200Zm40-100Z"/></svg>';
...
function loadProfileIntoForm(id) {
    ...
    restoreProductSelection(p.product || '', p.customPath || '');
}

function restoreProductSelection(product, customPath) {
    document.querySelectorAll('input[name="product"]').forEach(r => { r.checked = (r.value === product); });
    const customGroup = document.querySelector('.custom-path-group');
    const customInput = document.getElementById('custom-path');
    if (product) {
        config.product = product;
        if (product === 'custom') {
            config.remotePath = customPath;
            customInput.value = customPath;
            customGroup.style.display = 'block';
        } else {
            config.remotePath = productPaths[product] || '';
            customInput.value = '';
...
async function saveCurrentProfile() {
    ...
    if (!name) { await showMessage(t('alertProfileName')); return; }
    if (!host || !username) { await showMessage(t('alertHostUser')); return; }
    const product = config.product || '';
    const customPath = product === 'custom' ? (config.remotePath || '') : '';
    const data = { name, host, username, password, port, protocol, basePath, product, customPath };
    if (currentProfileId) {
    ...
    await showMessage(t('alertProfileSaved'), 'info');
}
...
async function exportProfiles() {
    if (profiles.length === 0) { await showMessage(t('alertNoProfiles')); return; }
    try {
        const filePath = await save({ defaultPath: 'iconup-profiles.json', filters: [{ name: 'JSON', extensions: ['json'] }] });
        if (filePath) { await invoke('export_profiles', { profiles, filePath }); await showMessage(t('alertExported'), 'info'); }
    } catch (e) { await showMessage('Error: ' + e, 'error'); }
}
    ...
    try {
        ...
        if (filePath) {
            ...
            if (imported && imported.length > 0) {
                ...
                renderProfileSelect();
                renderProfileList();
                await showMessage(t('alertImportedN', { n: imported.length }), 'info');
            } else await showMessage(t('alertNoProfilesFound'));
        }
    } catch (e) { await showMessage('Error: ' + e, 'error'); }
}
...
function initEventListeners() {
    ...
    document.getElementById('btn-retry-upload').addEventListener('click', () => { resetUploadUI(); startUpload(); });
...
function updateNavigation() {
    ...
    if (currentStep === totalSteps) btnNext.style.display = 'none';
        ...
        if (currentStep === 3) btnNext.innerHTML = `${t('btnStartUpload')} ${SVG_CHEVRON_RIGHT}`;
        else btnNext.innerHTML = `<span data-i18n="btnNext">${t('btnNext')}</span> ${SVG_CHEVRON_RIGHT}`;
    }
...
async function validateFtpConfig() {
    const host = document.getElementById('ftp-host').value.trim();
    ...
    const protocol = document.getElementById('ftp-protocol').value;
    const basePath = document.getElementById('ftp-basepath').value.trim();
    if (!host) { await showMessage(t('alertHost')); return false; }
    if (!user) { await showMessage(t('alertUser')); return false; }
    if (!pass) { await showMessage(t('alertPass')); return false; }
    config.ftp = { host, username: user, password: pass, port: parseInt(port) || 22, protocol, basePath };
    return true;
}

async function validateProductSelection() {
    if (!config.product) { await showMessage(t('alertProduct')); return false; }
    if (config.product === 'custom' && !config.remotePath) { await showMessage(t('alertCustomPath')); return false; }
    return true;
}

async function validateFolderSelection() {
    if (!config.localFolder) { await showMessage(t('alertFolder')); return false; }
    return true;
}
...
function showFolderPreview(files) {
    ...
    fl.innerHTML = display.map(f => `<div class="file-row">${SVG_FILE} <span>${escapeHtml(f)}</span></div>`).join('');
...
async function startUpload() {
    ...
    const badge = document.getElementById('upload-folder-badge');
    const folderName = config.localFolder ? config.localFolder.replace(/[\\/]+$/, '').split(/[\\/]/).pop() : '';
    if (folderName) {
...
function updateProgress(current, total, filename, status) {
    ...
    if (status === 'success') uploadSuccess++; else uploadErrors++;
    const safeName = escapeHtml(filename);
    addLog(status === 'success' ? `${SVG_CHECK} ${safeName}` : `${SVG_CLOSE} ${safeName}`, status, true);
}

function addLog(msg, type = 'info', html = false) {
    const log = document.getElementById('upload-log');
    const e = document.createElement('div');
    e.className = `log-${type} log-row`;
...
function showUploadComplete(data) {
    const hasErrors = uploadErrors > 0;
    ...
    if (hasErrors) {
        document.getElementById('upload-status').textContent = t('uploadCompleteErrors');
        iconEl.innerHTML = SVG_RESULT_WARNING;
        iconEl.classList.add('warning');
        titleEl.textContent = t('uploadCompleteErrorsTitle');
        document.getElementById('complete-message').textContent = t('uploadFilesErrors', { ok: uploadSuccess, errors: uploadErrors });
        retryBtn.style.display = 'inline-flex';
        addLog(t('uploadLogErrors', { errors: uploadErrors, total: data.total_files }), 'error');
    } else {
        document.getElementById('upload-status').textContent = t('uploadComplete');
        iconEl.innerHTML = SVG_RESULT_CHECK;
        iconEl.classList.remove('warning');
        titleEl.textContent = t('uploadCompleteTitle');
    ...
    btnPrev.innerHTML = `${SVG_CHEVRON_LEFT} ${t('btnNewInstall')}`;
...
function resetUploadUI() {
    ...
    document.getElementById('result-icon').classList.remove('warning');
    document.getElementById('btn-retry-upload').style.display = 'none';
    document.getElementById('upload-folder-badge').style.display = 'none';
    btnPrev.innerHTML = `${SVG_CHEVRON_LEFT} <span data-i18n="btnBack">${t('btnBack')}</span>`;
